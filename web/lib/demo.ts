...
/**
 * Is this a seeded sample rather than a real sealed task?
 *
 * Sample records keep their `demo-` id when they are seeded into a real
 * database, so the UI can mark them wherever they appear. That matters more
 * here than in most products: Rubric's entire claim is that what you see is
 * what was sealed on-chain, and a row that looks like a settled task but has no
 * escrow behind it would be the precise lie the protocol exists to prevent.
 *
 * Keyed off the id rather than a column so seeding needs no schema change, and
 * so a real task can never accidentally be marked as a sample - ids are
 * generated by cuid() and never start with this prefix.
 */
export function isSampleTask(id: string): boolean {
